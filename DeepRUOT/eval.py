...
from DeepRUOT.models import ODEFunc
...
def generate_trajectories_sde(df, f_net, sf2m_score_model, device, exp_dir, all_times, sigma, use_mass, num_points=None, num_runs = 1):
    ...
    for run_idx in tqdm(range(num_runs)):
        ...
        for i in range(1, len(all_times)):  # 时间点
            ...
            gt_mass = len(df[df['samples'] == i]) / len(df[df['samples'] == 0])
            model_i_data = sde_point_tensor[i]
            model_i_data = model_i_data.float()
