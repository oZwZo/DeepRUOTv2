...
class TrainingPipeline:
    ...
    def train_score_model(self):
        ...
        if sigma == 0:
            self.logger.info('Sigma=0, skip training score model')
    ...
    def evaluate(self):
        ...
        self.f_net.load_state_dict(torch.load(os.path.join(self.exp_dir, 'model_final'), map_location=self.device))
        self.sf2m_score_model.load_state_dict(torch.load(os.path.join(self.exp_dir, 'score_model_final'), map_location=self.device))
        ...
        if sigma == 0:
            ...
            num_runs = 5
        results = generate_trajectories_sde(self.df, self.f_net, self.sf2m_score_model, self.device, self.exp_dir, self.groups, sigma, use_mass=self.config['use_mass'], num_points=None, num_runs=num_runs)
        ...
        df_aggregated.to_csv(os.path.join(self.exp_dir, 'evaluation_result.csv'), index=False)
...
def main():
    ...
    return pretrain_losses, final_losses, evaluate_results
